...
export function useBudgetImportModal(onOpenChange: (open: boolean) => void) {
  ...
  const parseFile = useCallback(async (selectedFile: File) => {
    ...
    setIsParsing(true)
    try {
      const parsedRows = await parseBudgetFile(selectedFile)
      setRows(parsedRows)
    } catch (err: unknown) {
  ...
  const handleDownloadTemplate = useCallback(async (type: 'xlsx' | 'json') => {
    try {
      await downloadBudgetTemplate(type)
    } catch (err) {
