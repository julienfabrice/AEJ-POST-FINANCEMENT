...
export function useQuestionsGrid(searchQuery: string) {
    ...
    const { mutate: deleteMutation } = questionServices.useDelete()
  ...
  const columnDefs = useMemo<ColDef[]>(() => {
    return [
      ...
      // { field: 'cible_ind', headerName: 'Public cible', flex: 1, cellRenderer: (params: any) =>
      //         <Badge className="bg-[#2D6BD4] hover:bg-[#2D6BD4] font-mono text-white">{params.data.formulaire.public_cible}</Badge>
      // } ,
