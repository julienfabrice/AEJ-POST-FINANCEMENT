...
export function ParametresPage() {
  ...
  const fieldsDisabled = !isEditMode || isPending

  const handleToggleEditMode = (checked: boolean) => {
    if (!checked) form.reset() // annule les modifications non enregistrées en sortant du mode édition
    setIsEditMode(checked)
  }

  const handleSubmit = form.handleSubmit((values) => {
    onSubmit(values, () => setIsEditMode(false))
  })
