...
interface ProcedureGuichetCardProps {
  guichet: GUICHET_T
  onEdit?: () => void
  onDelete?: () => void
}

export function ProcedureGuichetCard({ guichet: g, onEdit, onDelete }: ProcedureGuichetCardProps) {
  return (
      ...
      <CardContent >
        <div className="flex items-center gap-[10px] mb-3">
          <div 
            ...
            style={{ backgroundColor: g.couleur || '#3498db' }}
          >
            <Workflow className="w-4 h-4" />
          </div>
          <div className="flex-1 min-w-0 leading-tight">
            <b className="text-[15px] text-[#2D6BD4] font-bold block truncate">{g.libelle}</b>
            <div className="font-mono text-xs text-[#8595A8] mt-0.5">
              {g.code} {g.workflow_code && `· ${g.workflow_code}`}
            </div>
          </div>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[200px]">
                <DropdownMenuItem className="cursor-pointer" asChild>
                  <Link to="/guichet-workflow/$workflowId" params={{ workflowId: g.workflow?.id?.toString() || '1' }} className="flex items-center gap-2 w-full">
        ...
        <div className="mt-3.5">
          <Link 
            ...
            params={{ workflowId: g.workflow?.id?.toString() || '1' }}
